...
const Profile = () => {
  ...
  const fetchUser = async (userId: string | number): Promise<UserProfile> => {
    const id = typeof userId === "string" ? parseInt(userId) : userId;
    if (userCache.has(id)) {
      return userCache.get(id)! as UserProfile;
    }
    try {
      ...
      userCache.set(id, userDetails);
      profileDataCache.user = userDetails;
      return userDetails;
    } catch (err) {
  ...
  const fetchUsers = async (): Promise<User[]> => {
    try {
      ...
      return users.filter((u) => u.id && u.username && u.email);
    } catch (err) {
  ...
  const fetchUserPosts = async (userId: number, currentUserId: number) => {
    ...
    try {
      ...
      if (validPosts.length === 0) console.warn("No valid posts after processing.");
      return validPosts;
    } catch (err) {
  ...
  const fetchCommentedPosts = async (userId: number, currentUserId: number) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/comments/my-comments/${userId}`, {
        method: "GET",
        credentials: "include",
      });
      if (!res.ok) throw new Error(`Failed to fetch commented posts: ${res.status}`);
      const commentedList: {
        id: number;
        content: string;
        imageUrl: string | null;
      ...
      if (validComments.length === 0) console.warn("No valid commented posts after processing.");
    } catch (err) {
  ...
  const fetchLikedPosts = async (userId: number, currentUserId: number) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/likes/my-likes/${userId}`, {
        method: "GET",
        credentials: "include",
      });
      ...
      if (validLikes.length === 0) console.warn("No valid liked posts after processing.");
    } catch (err) {
  ...
  const fetchBookmarkedPosts = async (userId: number, currentUserId: number) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/bookmarks/my-bookmarks/${userId}`, {
        credentials: "include",
      });
      if (!res.ok) throw new Error(`Failed to fetch bookmarked posts: ${res.status}`);
      ...
      if (validBookmarks.length === 0) console.warn("No valid bookmarked posts after processing.");
    } catch (err) {
  ...
  const handleTabChange = async (value: string, userId: number, currentUserId: number) => {
    if (value === "posts" && !fetchedTabs.posts) {
      ...
      await fetchLikedPosts(userId, currentUserId);
    } else if (value === "bookmarks" && !fetchedTabs.bookmarks) {
      await fetchBookmarkedPosts(userId, currentUserId);
    }
  };
