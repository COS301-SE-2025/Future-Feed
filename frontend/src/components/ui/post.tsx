...
const Post: React.FC<PostProps> = ({
  ...
  return (
    <Card
      className={cn(
        "future-feed:bg-card future-feed:text-white text-black border-2 cursor-pointer relative group",
        className
      )}
      onClick={handleClick}
          ...
          <div className="flex-1">
            <div className="flex justify-between items-center">
              ...
              <div className="flex items-center gap-2">
                <span className={`text-xs sm:text-sm whitespace-nowrap transition-all duration-200 ${isHovered && currentUser && currentUser.id === authorId ? 'mr-8' : 'mr-0'}`}>
                  {time}
                </span>
